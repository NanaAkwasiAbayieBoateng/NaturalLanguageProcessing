...
str_extract_all(x, pt)
str_extract_all(x, pt1)

str_extract_all(x, phone)
...
string<-c("Posted 69 months ago (7/4/2011)","2017-12-21 is todays date")
...
sentences %>%
  head(5) %>% 
  str_split(" ", simplify = TRUE)%>%head(5)
```
...
str_detect(sentences, fixed("the"))%>%head()
str_detect(sentences, "the")%>%head()
...
(?!...): negative look-ahead assertion. Matches if ... does not matche at the current input.
